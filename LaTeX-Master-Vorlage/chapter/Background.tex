...
\todoredefined[inline]{
TODO: Include own image instead of the one here? Other example than translation? \\
Include this? \\
...
The next quantity, I will use in the upcoming chapters, is the deformation gradient \textbf{F}. With its help, we can calculate the change in volume and length of an object during a deformation. We can obtain \textbf{F} by taking the gradient of the function $\phi$ discussed in the last section.
...
The resulting deformation gradient is then calculated by
\[
...
\]
If \textbf{F} is equal to the identity matrix \textbf{I}, there is no deformation present. This would be the case, if we look at rigid body displacements.
Since we will be handling deformation in the 3D space, the deformation gradient will have the form of a $3 \times 3$ matrix: 
...
\textbf{U} and $\mathbf{V}$ are both orthogonal matrices that represent the rotation of \textbf{F}. $\Sigma$ on the other hand indicates the scaling of each coordinate $x_i$ by the factor $\sigma_i$. Unlike the standard convention, where a possible reflection lies in the rotation variants (meaning \textbf{U} and \textbf{V}), here the reflections are moved to $\mathbf{\Sigma}$ and it is allowed to have a negative entry.
...
The right Cauchy-Green tensor \textbf{C} can be calculated by multiplying \textbf{F} with itself:
\[
\mathbf{C} = \mathbf{F^\intercal F}.
\]
It serves as a direct measurement of the deformation. 
