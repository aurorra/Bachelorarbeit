...
If we apply a force over an object, the object itself undergoes a deformation. This behavior is intuitively clear. But how can we formalize it?

Graphically, we can imagine a deformation with the help of a two dimensional deformation map as shown in Fig. \ref{fig:deformationmap}. The ellipse on the left side represents an object or material in its rest state. A function $\phi$ maps this rest state of the ellipse to a deformed state as shown on the right side.
...
This means that we can map each particle of a chosen object from its rest state (at time $t_0 = 0$) to a deformed one. Each particle $X$ of a body can be characterized by a vector $\mathbf{x}$ containing its coordinates. If the particles are displaced after a time $t$, we can describe their new coordinate vector with 
\[
	\check{\mathbf{x}} = \phi (\mathbf{x}, t).
\]
If we for example consider a \textit{Translation}, every particle is displaced by the same distance and direction. Thus we can calculate $\check{\mathbf{x}}$ with
\[
...
\todoredefined[inline]{
TODO: Include own image instead of the one here? \\
Include this? \\
...
The deformation gradient \textbf{F} can be obtained by deriving $\phi$. It is therefore the derivative of...

\begin{wrapfigure}{r}{5.5cm}
\centering
	\includegraphics[width=6.5cm]{resources/stretch_plot.png}
	\caption{Stretching of a rectangle}
	\label{stretch:1}
\end{wrapfigure} 

We can take a simple deformation in 2D to illustrate \textbf{F}. We can stretch an area in the following way:
\[
	Here comes formulas
\]
This is illustrated in \ref{stretch:1}.
