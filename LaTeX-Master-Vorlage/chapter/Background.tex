...
\todoredefined[inline]{
TODO: Not done yet. Include what is used later
}
...
The $\sigma_i$ denote the singular values of $\mathbf{\Sigma}$. Unlike the normal convention, \textbf{U} and \textbf{V} are rotations, which results in $\operatorname{det}(\mathbf{U})$ and $\operatorname{det}(\mathbf{V})$ being equal to 1. In addition, $\mathbf{\Sigma}$ is allowed to have negative entries.
...
\subsubsection{Right Cauchy-Green}

\subsubsection{First right Cuachy-Green invariant}
